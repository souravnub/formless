        ...
        {forms.map((form) => {
          ...
          return (
            <TableRow key={form.id} className="hover:bg-muted/40">
              ...
              <TableCell>
                <span className="block font-semibold">{form.user.name}</span>
                <Button
                  variant={"link"}
                  asChild
                ...
                <Button>
                  {foundSubmission ? (
                    <Link
                      href={
                        session?.user.role === "USER"
                          ? `/user/submissions/${foundSubmission.id}`
                          : `/supervisor/submissions/${foundSubmission.id}`
                      }
                    >
                      View
                    </Link>
                    ...
                    <Link
                      href={
                        session?.user.role === "USER"
                          ? `/user/forms/${form.id}/submissions/new`
                          : `/supervisor/forms/${form.id}/submissions/new`
                      }
                    >
                      Submit
                    </Link>
